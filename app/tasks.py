...
async def response_json(client: httpx.AsyncClient, url: str):
    ...
    async with limit_sem:
        ...
        if response.status_code == status.HTTP_307_TEMPORARY_REDIRECT:
            raise ValueError(f"{response.url.path} should have '/' in the end of path")
...
async def task(
    ...
    async with httpx.AsyncClient() as client:
        tasks: list[Any] = [
            response_json(client, prepare_url(url, _)) for _ in range(parallel_number)
        ]
...
async def common_save(
    ...
    data_path: Path = Path(f"{settings.BASE_PATH}") / "data" / (path or "demo")
    data_path.mkdir(parents=True, exist_ok=True)
    if data_path.exists():
        if not is_empty(data_path):
            [f.unlink() for f in data_path.glob("*") if f.is_file()]
        for idx, _data in enumerate(data):
            with (data_path / f'{(prefix or "demo")}.{idx}.json').open(
                "w", encoding="utf-8"
            ) as f:
...
async def merge_save(
    ...
    if data_path.exists():
        if not is_empty(data_path):
            [f.unlink() for f in data_path.glob("*") if f.is_file()]
        with (data_path / f'{(prefix or "demo")}.json').open(
            "w", encoding="utf-8"
        ) as f:
            f.write(
                json.dumps(
...
async def delete_files_and_empty_folders(
    ...
    for file in filter(lambda x: x.is_file(), data_path.rglob("*")):
        if datetime.fromtimestamp(file.stat().st_ctime, tz=timezone.utc) < (
            datetime.now(tz=timezone.utc) - settings.FILE_RETENTION_TIMEDELTA
        ):
