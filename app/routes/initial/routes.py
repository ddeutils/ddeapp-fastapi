...
@routes.get("/demo/data/", response_class=JSONResponse)
...
@routes.post("/demo/demo-job/", response_class=JSONResponse)
async def start_demo_job(
    ...
    data = await task(
        url=(f"{params.p_prcs_obj.server_nm}?limit={limit}"), parallel_number=parallel
    )
